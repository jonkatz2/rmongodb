...
typedef bson_t bson_buffer;

SEXP mongo_bson_destroy(SEXP b) {
    bson_t* _b = _checkBSON(b);
    bson_destroy_old(_b);
...
bson_iter_t* _checkIterator(SEXP iter) {
    ...
    bson_iter_t* _iter = (bson_iter_t*)R_ExternalPtrAddr(ptr);
    if (!_iter)
...
SEXP _mongo_code_w_scope_create(const char* code, bson_t* b) {
    SEXP ret, cls, _b;
    PROTECT(ret = allocVector(STRSXP, 1));
    SET_STRING_ELT(ret, 0, mkChar(code));
...
static void bsonIteratorFinalizer(SEXP ptr) {
    if (!R_ExternalPtrAddr(ptr)) return;
    bson_iter_t* iter = (bson_iter_t*)R_ExternalPtrAddr(ptr);
    Free(iter);
    R_ClearExternalPtr(ptr); /* not really needed */
}


SEXP _mongo_bson_iterator_create(bson_iter_t* iter) {
    SEXP ret, ptr, cls;
    PROTECT(ret = allocVector(INTSXP, 1));
    INTEGER(ret)[0] = 0;
    bson_iter_t* _iter = Calloc(1, bson_iter_t);
    memcpy(_iter, iter, sizeof(bson_iter_t));
...
int _mongo_bson_find(SEXP b, SEXP name, bson_iter_t* iter) {
    bson_t* _b = _checkBSON(b);
...
int _iterator_getComplex(bson_iter_t* iter, Rcomplex* z) {
...
SEXP _array_to_object(bson_iter_t* iter) {
    bson_type sub_type;
    bson_type common_type;
    Rcomplex z;
    bson_iter_t sub[MAXDIM+1];
    int dim[MAXDIM+1];
...
SEXP _mongo_bson_to_list(bson_t* b);

SEXP _mongo_bson_value(bson_iter_t* _iter) {
    SEXP ret;
    Rcomplex z;
    bson_type t = bson_iterator_type(_iter);
    switch (t) {
...
SEXP mongo_bson_iterator_value(SEXP iter) {
    bson_iter_t* _iter = _checkIterator(iter);
...
SEXP mongo_bson_buffer_append_bson(SEXP buf, SEXP name, SEXP value) {
    bson_buffer* _buf = _checkBuffer(buf);
    const char* _name = CHAR(STRING_ELT(name, 0));
    bson_t* _value = _checkBSON(value);
    SEXP ret;
    PROTECT(ret = allocVector(LGLSXP, 1));
...
SEXP mongo_bson_buffer_append_element(SEXP buf, SEXP name, SEXP value) {
    ...
    bson_iter_t* _value = _checkIterator(value);
    SEXP ret;
    PROTECT(ret = allocVector(LGLSXP, 1));
